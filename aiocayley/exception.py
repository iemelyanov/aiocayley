...
class TransportError(CayleyException):
    """Transport error.

    Exception raised when Cayley returns a non-OK (>=400) HTTP status
    ...
    @property
    def info(self):
        """Dict of returned error info from Cayley, where available."""
        return self.args[2]
...
class ConnectionError(TransportError):
    ...
    Cayley. Original exception from the underlying Connection
